...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!searchInput || !vagasCountInfo || !tabContent || tabs.length === 0) {
        console.error("Erro: Elementos essenciais do dashboard não encontrados (searchInput, vagasCountInfo, tabContent, tabs).");
    ...
    function updateVagasCountInfo(activeTabPane, visibleCount) {
        ...
        if (tabId === 'ativas-content') { statusText = 'ativas'; }
        else if (tabId === 'pausadas-content') { statusText = 'pausadas'; }
        else if (tabId === 'arquivadas-content') { statusText = 'arquivadas'; }
