...
document.addEventListener('DOMContentLoaded', () => {
    // ============================================================
    // 1. MÁSCARAS E VALIDAÇÕES DO MODAL 1 (EDITAR PERFIL)
    // ============================================================
    try {
    ...
    // Atualiza cidades ao trocar o estado
    ...
    const atualizarEnderecoCompleto = () => {
        ...
        // Cria o objeto com os dados do endereço
        const enderecoObj = {
            rua: ruaInput.value || "",
            ...
            uf: estadoSelect.value || "",
            cep: cepInput.value || ""
        };

        // Converte o objeto para JSON e salva no campo hidden
        enderecoHiddenInput.value = JSON.stringify(enderecoObj);

        // Monta a string de exibição
        const displayPartes = [
            enderecoObj.rua,
            enderecoObj.numero ? `, N°${enderecoObj.numero}` : "",
            enderecoObj.complemento ? ` - ${enderecoObj.complemento}` : "",
            enderecoObj.bairro ? ` - ${enderecoObj.bairro}` : "",
            (enderecoObj.cidade && enderecoObj.uf)
                ? `, ${enderecoObj.cidade}/${enderecoObj.uf}`
                : (enderecoObj.cidade || enderecoObj.uf),
            enderecoObj.cep ? ` (CEP: ${enderecoObj.cep})` : ""
        ];

        enderecoDisplayInput.value = displayPartes
            .filter(p => p && p.trim() !== "")
            .join("");
    };
