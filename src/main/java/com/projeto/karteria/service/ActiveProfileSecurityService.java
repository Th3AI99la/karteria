...
public class ActiveProfileSecurityService {
    ...
    public boolean hasActiveRole(String requiredProfile) {
        ...
        if (!(activeProfileAttr instanceof TipoUsuario)) {
            logger.error("ERRO: hasActiveRole - Atributo 'perfilAtivo' não é do tipo TipoUsuario. Tipo encontrado: {}",
        ...
        logger.debug("Resultado da comparação (Perfil da Sessão [{}] == Perfil Requerido [{}]) -> {}",
                activeProfile.name(), requiredProfile, hasRole);
